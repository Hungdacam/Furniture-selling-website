...
function renderProduct() {
    ...
    const content = product.map((item, index) => {
        if (index >= start && index < end) {
            html += `<div id="${item.id}" class="col" onclick="getDataProduct(${item.id})">
                        <div class="detail-item"><a href = "/DetailProduct.html"><img src = ${item.image}></a>
...
if(document.getElementById('number-page')){
...
function getDataProduct(detailItem){
    const itemDetail = product.filter(item => item.id === detailItem)
    localStorage.setItem('detail', JSON.stringify(itemDetail)) 
}
